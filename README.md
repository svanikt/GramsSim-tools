...
| [weights.py](scripts/weights.py) | Compute the total integrated flux `S` directly from the GramsSky input FITS maps and derive per-particle weight factors converting event counts into physical rates (Hz). Saves to `weights.pkl` inside your MAP_DIR. |
| [extract_df.py](scripts/extract_df.py) | Extract per-event quantities from all GramsSim ROOT trees (loaded as friends) into a single `events.parquet`. |
